...
/*
 * Capability description, not a promise that an external KPM loader is
 * attached. Userspace must still probe before enabling load controls.
 */
...
/* ABK-specific extensions (not present on stock ApkeSU kernels; guarded by ioctl return). */
